...
Follow these steps to get the intent classification system up and running on your local machine:
...
```bash
uvicorn intent_recognition:app --reload
```
...
```bash
uvicorn intent_recognition:app --reload --host 0.0.0.0 --port 8888
```
...
---
Certainly! Here's a detailed technical overview of the files present in the "Intent_Retrieval" repository:

---

## Repository Structure & Detailed File Descriptions
...
The generated (intent, example) pairs are saved in `intents.json` and the (intent, utterance) pairs in `utterances.json`.
...
1. **Injecting Intents and Examples:** 
   ...
   - **Steps:** 
     - a. Ensure that the `intents.json` (containing intent and example pairs) is present in the repository.
     - b. Run the `Data_injector.py` script. This script sends the intent-example pairs from `intents.json` to the `add_intent` endpoint of the deployed application, thereby populating the intent database.
...
2. **Evaluating Post-Deployment Performance:** 
   ...
   - **Steps:** 
     - a. Run the `Post_Evaluation.py` script. This script:
